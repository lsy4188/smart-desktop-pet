...
void init_MAX30102(void){
	...
	Write_MAX30102_Reg(MAX30102_REG_SPO2_CONFIG,0x29);
	// LED脉冲幅度：红光=3.0mA，红外光=3.0mA(电流过高可能光线太强导致光电二极管饱和（信号达到最大值））
    Write_MAX30102_Reg(MAX30102_REG_LED1_PA, 0x32); 
    Write_MAX30102_Reg(MAX30102_REG_LED2_PA, 0x32);
    // 多LED控制：启用LED1红光和LED2红外光
   	Write_MAX30102_Reg(REG_PILOT_PA,0x7f);
...
void get_SpO2_HR(void){
	...
	for(buffer_index=400;buffer_index<BUFFER_SIZE;buffer_index++){
		...
		if(heart_rate_valid==1&&heart_rate<120){
			dis_hr = heart_rate;
			dis_spo2 = spo2;
		}else{
			dis_hr = 0;
