	...
	switch(wb.connect_state){
		...
		case WIFI_STATE_CWJAP: 
			if (receve_wifi_response("OK")) {
				...
				wb.timeout_counter = 0;
				wb.retry_count=0;
				send_wifi_command("AT+CIPSTART=\"TCP\",\"192.168.20.171\",8080");
...
void control_device(char* device_name, char* state){
	...
	}else if(strcmp(device_name,"SHT20")==0){
		if(strcmp(state,"tem")==0){
			xTaskNotify(SHT20_TASK, BIT_0, eSetBits);
		}else{
			xTaskNotify(SHT20_TASK, BIT_1, eSetBits);
		}
	}else if(strcmp(device_name,"MAX30102")==0){
		if(strcmp(state,"SpO2")==0){
			xTaskNotify(MAX30102_TASK, BIT_0, eSetBits);
		}else{
			xTaskNotify(MAX30102_TASK, BIT_1, eSetBits);
		}
	}else{
