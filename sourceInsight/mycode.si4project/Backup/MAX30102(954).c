...
void init_MAX30102(void){
	//初始化i2c
	init_i2c();
	Write_MAX30102_Reg(MAX30102_REG_MODE_CONFIG, 0x40); // 复位
    while(Read_MAX30102_Reg(MAX30102_REG_MODE_CONFIG) & 0x40); // 等待复位完成
	systick_ms(10);
	printf("After reset: OVF=0x%02X\n", Read_MAX30102_Reg(MAX30102_REG_MODE_CONFIG));
	//FIFO指针清零
	...
	Write_MAX30102_Reg(MAX30102_REG_SPO2_CONFIG,0x24);
...
void Read_MAX30102_FIFOV1(uint32_t *red, uint32_t *ir){
	uint8_t wr_ptr, rd_ptr, ovf,fifo_level,new_rd_ptr,temp1;
	int i;
	uint8_t data[6];
	
	ovf = Read_MAX30102_Reg(MAX30102_REG_OVF_COUNTER);
	printf("OVF_COUNTER =  0x%x\r\n", Read_MAX30102_Reg(MAX30102_REG_OVF_COUNTER));
    if(ovf ==0x1F) {
         printf("FIFO locked due to overflow! Resetting...\n");
        // 强制重置FIFO
        Write_MAX30102_Reg(MAX30102_REG_OVF_COUNTER, 0x00); // 清除溢出计数
        Write_MAX30102_Reg(MAX30102_REG_FIFO_WR_PTR, 0x00);
        Write_MAX30102_Reg(MAX30102_REG_FIFO_RD_PTR, 0x00);
        *red = 0;
        *ir = 0;
	...
	ovf=Read_MAX30102_Reg(MAX30102_REG_OVF_COUNTER);
	printf("ovf: 0x%x\r\n", ovf);
	fifo_level=(wr_ptr-rd_ptr+32)%32;
	...
	temp1=Read_MAX30102_Reg(MAX30102_REG_INT_STATUS1);
	...
	if(fifo_level>0){
		for (i=0; i<6; i++) {
		 ...
		 //读取后应自动更新的指针
        new_rd_ptr = Read_MAX30102_Reg(MAX30102_REG_FIFO_RD_PTR);
        printf("New RD_PTR should be: 0x%x (expected 0x%x)\n", 
               new_rd_ptr, (rd_ptr + 1) % 32);

        *red = ((data[0] << 16) | (data[1] << 8) | data[2]) >> 6;
        *ir  = ((data[3] << 16) | (data[4] << 8) | data[5]) >> 6;
       /* // 更新指针（硬件自动递增RD_PTR，需重新读取）
        wr_ptr = Read_MAX30102_Reg(0x04);
