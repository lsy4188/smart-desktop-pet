...
void get_sht20_task(void * pvParameters){
	...
	init_SHT20V1();
	while (1){
		 printf("SHT20 task waiting for notification...\r\n");
        if(xTaskNotifyWait(0, 0xffffffff, &data, portMAX_DELAY) == pdTRUE){
            printf("SHT20 task received notification: %lu\r\n", data);
            
            if(xSemaphoreTake(xSemaphore, pdMS_TO_TICKS(100)) == pdTRUE){
                if(data == 1){
                    printf("Processing SHT20 data...\r\n");
                    Process_DataV1("hold");
                    printf("SHT20 data processing completed\r\n");
                }
                xSemaphoreGive(xSemaphore);
            } else {
