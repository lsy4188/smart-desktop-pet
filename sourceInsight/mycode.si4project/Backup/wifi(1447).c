...
uint8_t receve_wifi_responseV1(const char* expected){
	...
	if(usart3_isready){
		...
		if(cmd_ctrl!=NULL){
            if(sscanf(cmd_ctrl, "CTRL:%hhu,%hhu", &device_id, &state) == 2) {
...
void control_device(uint8_t device_id, uint8_t state){
	switch (device_id)
		{
		case 0: if(state)GPIO_SetBits(GPIOC, GPIO_Pin_4);
				else GPIO_ResetBits(GPIOC, GPIO_Pin_4);
			break;
		case 1: if(state)GPIO_SetBits(GPIOA, GPIO_Pin_15);
				else GPIO_ResetBits(GPIOA, GPIO_Pin_15);
				break;
		case 3: if(state)GPIO_SetBits(GPIOC, GPIO_Pin_6);
