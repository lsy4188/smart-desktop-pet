...
void parse_command(void){
	cache[31]='\0';
	if(!(strcmp((char*)cache,"led_open")|strcmp((char*)cache,"led_close"))){
	mode=LED;
	key_flag^=0x01;
	}else if(!(strcmp((char*)cache,"beep_open")|strcmp((char*)cache,"beep_close"))){
	mode=BEEP;	
	key_flag^=0x02;
	}else if(!(strcmp((char*)cache,"fan_open")|strcmp((char*)cache,"fan_close"))){
	mode=FAN;
	key_flag^=0x04;
	}else{
