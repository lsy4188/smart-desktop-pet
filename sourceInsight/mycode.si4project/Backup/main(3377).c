...
void task_master( void * pvParameters ){
	...
	xTaskCreate(task_one, "task_one", task1_STACK_SIZE, NULL, 0, &task1);
	xTaskCreate(task_two, "task_two", task2_STACK_SIZE, NULL, 1, &task2);
...
int main(void)
{
	...
	xTaskCreate(task_master, "task_master", STACK_SIZE, NULL, 0, &master);
	vTaskStartScheduler();
	while(1){
	}
}
