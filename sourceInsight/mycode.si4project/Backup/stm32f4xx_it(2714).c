...
void EXTI9_5_IRQHandler(void){
	...
	if((current_time-last_interrupt_time)>pdMS_TO_TICKS(300)){
		...
		if(EXTI_GetITStatus(EXTI_Line5)){
			...
			xTaskNotifyFromISR(KEY_DETECT_TASK, KEY3, eSetValueWithoutOverwrite, &pxHigherPriorityTaskWoken);
		}
		if(EXTI_GetITStatus(EXTI_Line8)){
			EXTI_ClearITPendingBit(EXTI_Line8);
			xTaskNotifyFromISR(KEY_DETECT_TASK, KEY2, eSetValueWithoutOverwrite, &pxHigherPriorityTaskWoken);
		}
		if(EXTI_GetITStatus(EXTI_Line9)){
			EXTI_ClearITPendingBit(EXTI_Line9);
			xTaskNotifyFromISR(KEY_DETECT_TASK, KEY1, eSetValueWithoutOverwrite, &pxHigherPriorityTaskWoken);
		}
