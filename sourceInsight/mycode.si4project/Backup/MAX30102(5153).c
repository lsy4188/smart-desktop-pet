...
void Read_MAX30102_FIFOV2(uint32_t *red, uint32_t *ir){
    ...
    uint8_t num_samples = (wr_ptr - rd_ptr) & 0x1F; // 32样本环形缓冲 
    if (num_samples > 0) {
        uint8_t data[6];
        MAX30102_ReadBytes(MAX30102_REG_FIFO_DATA, data, 6);
        *red = ((data[0] << 16) | (data[1] << 8) | data[2]) >> 6;
        *ir  = ((data[3] << 16) | (data[4] << 8) | data[5]) >> 6;
        // 更新读指针（硬件自动递增后需软件同步）
        uint8_t new_rd_ptr = (rd_ptr + 1) & 0x1F;
        Write_MAX30102_Reg(MAX30102_REG_FIFO_RD_PTR, new_rd_ptr);
    }
}
