...
void ParseCommand(void){
	char *res=NULL;
	cache[31]='\0';
	if(strcmp((char*)cache,"led_open")){
	mode=LED;
	key_flag^=0x01;
	}else if(strcmp((char*)cache,"beep_open")){
	mode=BEEP;	
	key_flag^=0x02;
	}else if(strcmp((char*)cache,"fan_open")){
	mode=FAN;
	key_flag^=0x04;
	}else{
		strcpy(res,"UNKNOW COMMAND!!!");
		USART_SendData(USART6, (u16)res);
