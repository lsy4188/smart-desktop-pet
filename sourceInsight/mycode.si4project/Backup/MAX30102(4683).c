...
void init_MAX30102(void){
	...
	//配置FIFO：采样平均=16,FIFO满阈=17
	Write_MAX30102_Reg(MAX30102_REG_FIFO_CONFIG,0x8F);
	// 模式配置：SpO2模式
    ...
    // 多LED控制：启用LED1红光和LED2红外光
	 ...
	 // 使能满数据就绪中断，环境光溢出中断，温度就绪中断
    Write_MAX30102_Reg(MAX30102_REG_INT_ENABLE1, 0xA0);
...
/*
检查环境光是否溢出(溢出会对数据造成影响)
*/
uint8_t Ambient_Light_Overflow(void){
	uint8_t flag=0;
		if(Read_MAX30102_Reg(MAX30102_REG_INT_STATUS1)&0x20){
			Read_MAX30102_Reg(MAX30102_REG_INT_STATUS1);//清除中断
			flag=1;
		printf("Ambient Light Overflow........\r\n");
		}
		return flag;
}
