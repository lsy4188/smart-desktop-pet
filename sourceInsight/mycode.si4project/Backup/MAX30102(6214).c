...
void init_MAX30102(void){
	...
	Write_MAX30102_Reg(MAX30102_REG_FIFO_CONFIG,0x4F);//采样平均=4，FIFO满阈=17
	// 模式配置：SpO2模式
	Write_MAX30102_Reg(MAX30102_REG_MODE_CONFIG,0x03);
	// SPO2配置：设置 SpO2 传感器中 ADC 的满量程范围4096,采样率100Hz，LED脉冲宽度=411μs (18位分辨率)
	Write_MAX30102_Reg(MAX30102_REG_SPO2_CONFIG,0x03);
   ...
   // Write_MAX30102_Reg(MAX30102_REG_MULTI_LED1, 0x21);
	 // 使能满数据就绪中断和温度就绪中断
    //Write_MAX30102_Reg(MAX30102_REG_INT_ENABLE1, 0x40);//新数据中断
...
uint8_t MAX30102_samples_data_is_ready(void){
	uint8_t flag=0;
	if(Read_MAX30102_Reg(MAX30102_REG_INT_STATUS1)){
...
uint16_t Get_MAX30102_samples(void){
	uint16_t num_samples=0,num_samples1=0;
	uint8_t wr_ptr,rd_ptr,ovf,wr_ptr1,rd_ptr1;
	if(!MAX30102_samples_data_is_ready()){
		printf("num_samples is 0........");
		wr_ptr1=Read_MAX30102_Reg(MAX30102_REG_FIFO_WR_PTR);
		rd_ptr1=Read_MAX30102_Reg(MAX30102_REG_FIFO_RD_PTR);
		num_samples1 = (wr_ptr - rd_ptr)& 0x1F;
		printf("num_samples is 0......wr_ptr ======>0x%x\r\n",wr_ptr1);
		printf("num_samples is 0......rd_ptr ======>0x%x\r\n",rd_ptr1);
		printf("num_samples is 0......num_samples1 ======>%d\r\n",num_samples1);
		return num_samples;
	}
	//判断FIFO是否满MAX30102_REG_OVF_COUNTER中记录了丢失的数据个数
	...
	printf("rd_ptr ======>0x%x\r\n",rd_ptr);
	num_samples = (wr_ptr - rd_ptr)& 0x1F; // 32样本（回绕情况）
	return num_samples;
}
/*
读取FIFO中的红外和红光数据
...
void Read_MAX30102_FIFO(uint32_t *red, uint32_t *ir){
	uint8_t num_samples;
	uint8_t data[6];
	num_samples=Get_MAX30102_samples();
