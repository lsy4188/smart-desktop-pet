...
GPIO_InitStruct.GPIO_Pin=GPIO_Pin_5;//C_KEY1 | C_KEY2 | C_KEY3;
...
u8 val=0;
if(!GPIO_ReadInputDataBit(GPIOC, C_KEY3)){
	systick_ms(20);//按键消抖
	if(!GPIO_ReadInputDataBit(GPIOC, C_KEY3)){
		while(!GPIO_ReadInputDataBit(GPIOC, C_KEY3));//长按
		key_flag=~key_flag;
		mode=1;
		val=1;
		return val;
...
if(!GPIO_ReadInputDataBit(GPIOF, F_KEY4)){
	systick_ms(20);//按键消抖
	if(!GPIO_ReadInputDataBit(GPIOF, F_KEY4)){
		while(!GPIO_ReadInputDataBit(GPIOF, F_KEY4));//长按
		key_flag=~key_flag;
		mode=2;
		val=1;
		return val;
...
if (!GPIO_ReadInputDataBit(GPIOC, C_KEY3)){
	systick_ms(20);
	if (!GPIO_ReadInputDataBit(GPIOC, C_KEY3)){
		while(!GPIO_ReadInputDataBit(GPIOC, C_KEY3));
		key_flag=~key_flag;
	if(key_flag){
		mode++;
		if (mode > num) {
